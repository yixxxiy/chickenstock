...
def main() -> None:
    ...
    panel = saved["panel-tall"]
    medium = saved["panel-medium"]
    close = pad_square(saved["seal-x-red"], 56)
    ...
    copy(saved["chip-toggle-off"], SIMPLE / "tag.png", SIMPLE / "chip-off.png")
    copy(saved["chip-toggle-on"], SIMPLE / "chip-on.png")
    copy(saved["deco-flourish"], SIMPLE / "flourish.png")
    copy(saved["icon-mail"], UI / "mail.png")
    copy(saved["icon-gear"], FARM / "settings_gear.png")
    copy(saved["bubble-speech"], FARM / "speech.png", FARM / "thought_bubble.png")
