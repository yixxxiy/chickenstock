...
func _ready() -> void:
	...
	assert(game.tutorial_step == 6 and game.shares == 1)
	assert(game.price == 150)
	assert(game.sell_shares(true))
	await get_tree().create_timer(1.0).timeout
	assert(game.tutorial_step == 7 and game.shares == 0)
	game._tutorial_next_pressed()
	...
	# 已知不稳定：这个测试大约每 4~6 次有一次在 quit 之后的引擎清理阶段段错误
	# （exit 139），OK 已经打出来了、断言全过，纯粹是退出时序。
	# 试过在这里多等 0.6 秒，没能消掉。放进 CI 前要么按输出里的 *_OK 判成败，
	# 要么先定位到底是哪一批 tween / 定时器引用了已释放的节点。
	get_tree().quit(0)
