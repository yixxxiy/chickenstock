...
static func grass_step(is_chick: bool, at: Vector2 = Vector2(48, 62)) -> void:
	if live:
		live._queue_step(is_chick, at)
...
func _queue_step(is_chick: bool, at: Vector2) -> void:
	...
	if randf() < skip:
		...
		return
	_grass_gate = randf_range(0.42, 1.2) if is_chick else randf_range(0.5, 1.45)
	_grass_t = 0.0
	_grass_kind = randi() % 3
	...
	match _surf:
		...
		3:
			...
			_tap_freq = randf_range(148.0, 240.0) if is_chick else randf_range(110.0, 175.0)
		4:
