...
func shake(n: Control) -> void:
	...
	_store(n, "shake", tw)
	var base := n.rotation
	tw.tween_property(n, "rotation", base - 0.12, 0.045)
	tw.tween_property(n, "rotation", base + 0.12, 0.07)
	tw.tween_property(n, "rotation", base - 0.08, 0.06)
	tw.tween_property(n, "rotation", base, 0.08).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
